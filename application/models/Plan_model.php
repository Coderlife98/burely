...
/*-------------------------Written by Amit end---------------------------*/
    ...
    public function find_placement_field($sponsor = "")
    {
        ...
        if (5 == 5) {
            ...
            $this->db->select("id,A,B,C,D,E")->from("msdr_members")->where('username',$sponsor);
			$this->db->group_start();
			$this->db->where("A", 0)->or_where("B", 0)->or_where("C", 0)->or_where("D", 0)->or_where("E", 0);
			$this->db->group_end();
            $result = $this->db->get()->row();
        ...
        return array("id" => $id, "leg" => $position);
    }
